...
def part2(_input: str) -> int:
	with open(_input, "r", encoding="UTF-8") as file:
		lines = file.readlines()
	priority_queue = []
	used_size, _ = calculate_dir_size(lines, len(lines), 1, heapq.heappush, priority_queue)
...
def calculate_dir_size_sum(total_sum: TotalSum, current_dir_size: int) -> None:
...
def calculate_dir_size(lines: list[str], len_lines: int, index: int, current_dir_size_function: Callable[[int, object], None], state_object: object) -> tuple[int, int]:
	...
	while True:
		if index >= len_lines:
			current_dir_size_function(state_object, current_dir_size)
